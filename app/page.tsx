...
export default function Page() {
  ...
  return (
    <main className="min-h-screen overflow-clip bg-background text-foreground">
      ...
      {copilot && <div className="overlay" onClick={() => setCopilot(false)}><aside className="copilot" onClick={e => e.stopPropagation()}><div className="copilot-header"><div><span className="section-kicker">PORTFOLIO GUIDE</span><h3>Explore my work</h3></div><button onClick={() => setCopilot(false)}><X size={18} /></button></div><div className="copilot-message"><div className="bot-icon"><Bot size={17} /></div><p>Hi, I can help you explore Oeurn&apos;s projects, technical stack, experience, and availability.</p></div><div className="quick-asks"><button onClick={() => setMessage('What is your strongest project?')}>Strongest project?</button><button onClick={() => setMessage('What is your stack?')}>What&apos;s the stack?</button></div><div className="copilot-input"><input value={message} onChange={e => setMessage(e.target.value)} placeholder="Ask about my work..." /><button onClick={() => setMessage('Thanks for your question. Let&apos;s connect to discuss it.') }><Send size={15} /></button></div></aside></div>}
    </main>
  )
}
